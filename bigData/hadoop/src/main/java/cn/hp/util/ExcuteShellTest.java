...
//
//
//    //    这里需要注意一个问题：通过远程连接到linux主机上时，在执行命令时，命令前面已经是他的完全路径，不能是简单的一个命令，否则会出现不认识在该命令。如　CustomTask sampleTask1 = new ExecCommand(" /home/grid/sqoop-1.4.5/bin/sqoop help")
////    或者在执行该命令之前，执行source /etc/profile。但是要注意这句话要跟所要执行的命令一起放在一起，用分号隔开。如下所示： （如果两个命令不放在一起，将会出现原来同样的错误）CustomTask sampleTask1 = new ExecCommand(" source /etc/profile; sqoop help")
//    @Test
