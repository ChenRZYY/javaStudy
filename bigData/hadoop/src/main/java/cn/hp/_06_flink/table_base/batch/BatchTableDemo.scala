...
object BatchTableDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val csvDataSet: DataSet[(String, String, String, String)] = env.readCsvFile[(String, String, String, String)]("./dataset/score.csv")
    val table2: Table = tableEnv.fromDataSet(csvDataSet,'id ,'name,'subjectId,'score)
    ...
    val table: Table = tableEnv.scan("tableA")
      ...
      .filter("name=='张三'")
    //tableEnv.sqlQuery("select id,name,subjecct,score from tableA where name='張三'")
    // 6. 打印表结构
    table.printSchema()
    // 7. 将数据落地到新的CSV文件中
    table.writeToSink(new CsvTableSink("./dataset/score_table.csv", ",", 1, FileSystem.WriteMode.OVERWRITE))
//    table.insertInto()
    // 8. 执行任务
    env.execute()
  }
