package cn.sdrfengmi.spark.project._01_hadoop_Offline.pageviews;

import cn.sdrfengmi.spark.project._01_hadoop_Offline.WebLogClean.WebLogBean;
...
class ClickStreamMapper extends Mapper<LongWritable, Text, Text, WebLogBean> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        v.setBean("true".equals(fields[0]) ? true : false, fields[1], fields[2], fields[3], fields[4], fields[5], fields[6], fields[7], fields[8]);
        //只有有效记录才进入后续处理
        if (v.isValid()) {
            //此处用ip地址来标识用户
...
class ClickStreamReducer extends Reducer<Text, WebLogBean, NullWritable, Text> {
    Text v = new Text();
...
//			}
        ...
        try {
             ...
             * 否则，就属于不同的session
             * TODO 每天 每个ip的会话数量,一天一个人有好几个会话,一个会话有很多网页
             *
             *
             */
