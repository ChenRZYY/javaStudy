...
package cn.sdrfengmi.batch_process.util;
 ...
 */
public abstract class SdrfengmiAbstractTableInputFormat<T> extends RichInputFormat<T, SdrfengmiTableInputSplit> {
	...
	@Override
	public void open(SdrfengmiTableInputSplit split) throws IOException {
		if (table == null) {
	...
	private void logSplitInfo(String action, SdrfengmiTableInputSplit split) {
	...
	@Override
	public SdrfengmiTableInputSplit[] createInputSplits(final int minNumSplits) throws IOException {
		if (table == null) {
			throw new IOException("The HBase table has not been opened! " +
				"This needs to be done in configure().");
		}
		...
		final List<SdrfengmiTableInputSplit> splits = new ArrayList<SdrfengmiTableInputSplit>(minNumSplits);
		for (int i = 0; i < keys.getFirst().length; i++) {
			...
			if ((scanWithNoLowerBound || isLastRegion || Bytes.compareTo(startRow, endKey) < 0) &&
				...
				int id = splits.size();
				final SdrfengmiTableInputSplit split = new SdrfengmiTableInputSplit(id, hosts, table.getName().getName(), splitStart, splitStop);
				splits.add(split);
			}
		}
		LOG.info("Created " + splits.size() + " splits");
		for (SdrfengmiTableInputSplit split : splits) {
			logSplitInfo("created", split);
		}
		return splits.toArray(new SdrfengmiTableInputSplit[splits.size()]);
	}
	...
	@Override
	public InputSplitAssigner getInputSplitAssigner(SdrfengmiTableInputSplit[] inputSplits) {
