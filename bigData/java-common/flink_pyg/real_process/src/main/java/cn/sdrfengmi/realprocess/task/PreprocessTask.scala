...
import cn.sdrfengmi.realprocess.bean.{ClickLogWide, Message}
  ...
  def process(watermarkDataStrem: DataStream[Message]): DataStream[ClickLogWide] = {
    watermarkDataStrem.map {
      msg =>
        ...
        val address: String = msg.clickLog.country + msg.clickLog.province + msg.clickLog.city
        //查看表中有访问时间字段,
