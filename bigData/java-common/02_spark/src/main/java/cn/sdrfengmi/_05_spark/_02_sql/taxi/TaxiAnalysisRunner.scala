package cn.sdrfengmi._05_spark._02_sql.taxi
...
object TaxiAnalysisRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //异常处理方式一
    //val taxiParsed3 = taxiRaw.rdd.map(row=>Try(parse(row))).filter(_.isSuccess).map(_.get)
    //异常处理方式二
    //val taxiParsed2 = taxiRaw.rdd.map(row=>safe3(parse,row))
...
//    taxiClean.show()
    ...
    val sortedFeatures: List[Feature] = featureCollection.features.sortBy((feature: Feature) => {
...
//      .show()
  ...
  def safe2[Row,Trip](f:Row=>Trip):Row=>Either[Trip,(Row,Exception)]={
    val func = (param:Row)=>{
      try{
        Left(f(param))
      }catch {
  ...
  def safe3(parse:Row=>Trip,row:Row)={
    try{
      Left(parse(row))
    }catch {
      case e:Exception=> Right(row,e)
    }
  }
