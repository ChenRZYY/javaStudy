...
//  Logger.getLogger("org").setLevel(Level.ERROR)
  ...
  def mapPartitions(): Unit = {
    ...
    sc.parallelize(Seq(1, 2, 3, 4, 5, 6, 7, 8))
      .mapPartitions[Int](iter => { //Int 类型可以写也可以不写,自动推断
      System.err.println(iter.toBuffer)
  ...
  def mapPartitions2(): Unit = {
    ...
    sc.parallelize(Seq(1, 2, 3, 4, 5, 6), 2)
      .mapPartitions(iter => {
        ...
        //        println(itera.length) //TODO 有两个分区每个分区的长度都为3
        //        itera
      })
      .collect()
      .foreach(re => {
  ...
  def mapPartitionsWithIndex(): Unit = {
    var buffer = sc.parallelize(Seq(1, 2, 3, 4, 5, 6), 4)
  ...
  def combineByKey(): Unit = {
    ...
    // 2. 算子操作
    //   2.1. createCombiner 转换数据,之作用于第一条数据
  ...
  def aggregateByKey(): Unit = {
    ...
    rdd.aggregateByKey(0.8)((agg, curr) => {
