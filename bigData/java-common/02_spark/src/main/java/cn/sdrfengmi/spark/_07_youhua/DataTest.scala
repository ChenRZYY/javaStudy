...
class DataTest extends Serializable {
  ...
  /**
    ...
    *    3、在groupby的时候，某一个key的数据特别多，该key的数据都会聚集在一个分区，导致数据倾斜----->随机数局部聚合,再全局聚合
    *    4、大表 join 小表，产生的结果中某一个key的数据量特别大，导致数据倾斜----------->小表广播出去/缓存，避免shuffle,从而避免数据倾斜
    *    5、大表1 join 大表2，大表1中存在个别key数据量特别大，导致数据倾斜-------------->1、将没有出现数据倾斜的key的数据照常处理,2、将处理数据倾斜key的数据添加随机数1-10，另一个表扩大10倍,这样前面每个表都能join上,3合并1,2的数据
    *    6、大表1 join 大表2，大表1中存在有很多的key数据量特别大，导致数据倾斜---------->直接将有数据倾斜key的表添加随机数1-10，将另一个表扩容10倍
    */
